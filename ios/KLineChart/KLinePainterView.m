...
-(void)drawLongPressCrossLine:(CGContextRef)context {
    ...
    CGContextSetLineWidth(context, ChartStyle_vCrossWidth);
...
-(void)drawRealTimePrice:(CGContextRef)context {
    ...
    if((-_scrollX - rect.size.width) > 0) {
        ...
        CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:_mainBackgroundColor].CGColor);
        CGContextDrawPath(context, kCGPathFill);
        [self.mainRenderer drawText:text atPoint:CGPointMake(self.frame.size.width - rect.size.width, y - rect.size.height / 2) fontSize:fontSize textColor:ChartColors_realTimeTextCloor];
        if(_isLine) {
            CGContextSetFillColorWithColor(context, [UIColor whiteColor].CGColor);
